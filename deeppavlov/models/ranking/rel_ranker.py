...
from deeppavlov.models.squad.utils import dot_attention, softmax_mask
...
class TwoSentencesEmbedder(Component):
    ...
    def __call__(self, sentence_tokens_1: List[str], sentence_tokens_2: List[str]):
...
class RelRanker(LRScheduledTFModel):
    ...
    def __init__(self, n_classes: int = 2,
        ...
        with tf.variable_scope("question_encode"):
            (h_fw, h_bw), (h_last_fw, h_last_bw) = cudnn_bi_gru(question_dr, 150, seq_lengths=q_len)
            q = tf.concat([h_fw, h_bw], axis=2)
        with tf.variable_scope("relation_encode"):
            (h_fw, h_bw), (h_last_fw, h_last_bw) = cudnn_bi_gru(self.rel_emb_ph, 150, seq_lengths=r_len)
